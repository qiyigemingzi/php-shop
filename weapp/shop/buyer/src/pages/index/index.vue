...
<view class="container">
  <swiper class="banner" indicator-dots="true" autoplay="true" interval="3000" duration="1000">
    <swiper-item v-for="item in banner" :key="item.ad_id">
      <navigator :url="item.ad_link">
        <img :src="item.ad_code" background-size="cover" />
      </navigator>
    </swiper-item>
  </swiper>
  <!--<view class="m-menu">-->
  ...
  <view class="a-section a-new" v-if="newGoods.length">
    ...
    <view class="b">
      <view class="item" v-for="item of newGoods" :key="item.goods_id">
        <navigator :url="'../goods/goods?id='+ item.goods_id">
          <img class="img" :src="item.original_img" background-size="cover" />
  ...
  <view class="a-section a-popular" v-if="hotGoods.length">
    ...
    <view class="b">
      <view class="item" v-for="item of hotGoods" :key="item.goods_id">
        <navigator :url="'/pages/goods/goods?id=' + item.goods_id">
          <img class="img" :src="item.original_img" background-size="cover" />
          <view class="right">
            <view class="text">
              <text class="name">{{item.goods_name}}</text>
              <text class="desc">{{item.goods_remark}}</text>
              <text class="price">￥{{item.shop_price}}</text>
            </view>
          </view>
        </navigator>
